  ...
  return (
    ...
    >
      ...
      <div className="flex sm:contents items-start justify-between gap-3">
        ...
        <div
          className={`sm:absolute sm:top-8 sm:right-8 flex flex-col items-end gap-3 transition-opacity duration-500 ${isRunning && !isPaused ? "opacity-20" : "opacity-100"}`}
        >
              ...
              {todayTodos.length === 0 ? (
                ...
                todayTodos.map((todo: Todo) => {
                  ...
                  const pct = Math.min(
                    100,
                    Math.floor(
                      (progressSeconds / 3600 / todo.targetHours) * 100,
                    ),
                  );
                  const done = pct >= 100;
